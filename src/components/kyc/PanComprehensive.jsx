...
import { GetUserPanReport } from "../services/Services_API";
    ...
    const report = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                toast.error(error?.response?.data?.errors?.id_number[0] || error.message || "Something went wrong!");
                console.log(error)
                setLoading(false);
                setIsReport(false);
                setRes({});
                ...
                <div className={`${isOpenSidebar && "lg:ml-64"} p-3 flex-1`}>
                    {!isReport && Object.keys(res).length === 0 && (
                        <div className="w-full mx-auto text-black  mt-2">
                            <div className="border border-green-300  md:w-2/3 mx-auto p-4 md:p-8 shadow-md rounded">
                                <div className="mb-6">
                                    <h2 className="text-xl font-semibold italic text-green-600">Pan Comprehensive</h2>
                                    <p className="text-xs italic mb-1">Pan Comprehensive Report</p>
                    ...
                    {isReport && Object.keys(res?.data) && (
                        <div className="w-full mx-auto text-black  mt-2">
                            ...
                            /> */}
                            <KycReport report={res.data} type="PanCompre" onGetNewReport={handleNewReport} />
                        </div>
                    )}
