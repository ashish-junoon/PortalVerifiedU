...
import { GetUserAadhaarReport } from "../services/Services_API";
...
function DrivingLicence() {
    ...
    const report = useFormik({
        ...
        validationSchema: Yup.object({
            DlNo: Yup.string().required('DL Number is required'),
        ...
        onSubmit: async (values) => {
            try {
                setLoading(true);
                const payload = {
                    dl_number: values.DlNo.toUpperCase(),
                    dob: values.dob,
                    url: 'verifiedu/VerifyDrivingLicence'
                }
                ...
                if (respose.success === true) {
                    ...
                    setLoading(false);
                    setRes({});
                    toast.error(respose?.verificationResult?.message || respose?.message || "Something went wrong!");
                }
            } catch (error) {
    ...
    return (
        <div>
            ...
            <div className="flex">
                ...
                <div className={`${isOpenSidebar && "ml-64"} p-3 flex-1`}>
                    {!isReport && Object.keys(res).length === 0 && (
                        <div className="w-full mx-auto text-black  mt-5">
                            <div className="relative overflow-hidden border border-gray-100  md:w-full mx-auto p-4 md:p-8 shadow-md rounded">
                                <div className="absolute bg-primary/10 w-50 h-50 md:top-[-60px] max-md:top-[-100px] right-[-60px] rounded-full"></div>
                                <div className="mb-6">
                                    <h2 className="text-xl font-semibold italic text-primary">Driving Licence</h2>
                                    <p className="text-xs italic mb-1">Driving Licence KYC Report</p>
                    ...
                    {isReport && Object.keys(res?.data) && (
                        <div className={`max-md:${isOpenSidebar && "ml-64"} mx-auto text-black  mt-4`}>
                            {/* <UserPrefillReport
                            ...
                            /> */}
                            <KycReport report={res?.data} type="DrivingLicence" onGetNewReport={handleNewReport} />
                        </div>
                    )}
